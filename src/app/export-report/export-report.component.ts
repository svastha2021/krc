...
export class ExportReportComponent implements OnInit {
  ...
  constructor(
    ...
    if (this.router.url.indexOf('invoice-payment-report')) {
  ...
  setTitle(){
    if(this.reportType === 'invoicepaymentreport'){
  ...
  getReports() {
    ...
    this.docReportService.retrieveData(this.reportType,from_date, to_date).subscribe((data) => {
