...
export class ExportReportComponent implements OnInit {
  ...
  constructor(
    private dp: DatePipe,
    private docReportService: ExportReportService,
    private us: UtilityService,
    private dateAdapter: DateAdapter<Date>,
    private router: Router
  ) {
    this.dateAdapter.setLocale('en-GB'); //dd/MM/yyyy
    if (this.router.url.indexOf('invoice-payment-report') > 0) {
      this.reportType = 'invoicepaymentreport';
      this.title = 'Invoice Payment Report';
    }
    if (this.router.url.indexOf('invoice-product-report') > 0) {
      this.reportType = 'invoiceproductreport';
      this.title = 'Invoice Product Report';
    }
  }
  ...
  getReports() {
    ...
    this.docReportService
      .retrieveData(this.reportType, from_date, to_date)
      .subscribe((data) => {
  ...
  export() {
    let fileName = this.reportType.toLocaleUpperCase();
    this.us.exportArrayToExcel(this.reportData, fileName);
  }
}
